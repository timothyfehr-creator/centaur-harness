...
Two routes `r1, r2`. Blockability is **presence-derived**: a route is blockable **iff** an
adjudicator-private `route_secret:{route}` entity exists for it (carrying its own `block_threshold`,
`int 0–99`, `ASSUMED`). The base scenarios carry only `route_secret:r1`, so **`r2` is unblockable there**;
a scenario that adds `route_secret:r2` (the "RED matters" game,
`examples/contested_logistics_both_blockable`) makes r2 blockable too. A d100 is consumed **iff** a
`BLOCK_ROUTE` targets a route that was `DISPATCH`-ed this turn **and** that route is blockable (has a
`route_secret`) — e.g. `dispatch r1 + block r1` in the base topology. **Block succeeds iff
`d100 < block_threshold`** (the dispatched route's own threshold; d100 ∈ **0–99**).

The table below is the **base single-secret** topology (only r1 blockable); with `route_secret:r2` present
the `r2` column mirrors the `r1` column (a contested `dispatch r2 + block r2` draws and can be lost).
...
d100 0–99; block iff `d100 < threshold`; total loss; quantity ∈ [1,30]; **blockability presence-derived**
(a route is blockable iff a `route_secret:{route}` exists; base scenarios carry only `route_secret:r1`, so
r2 is unblockable there); empty turn legal-and-committed; RED's failed block **not** observable to BLUE; golden vectors
